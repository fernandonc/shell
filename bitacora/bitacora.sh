...
if [ -f db1 ]; then
   ...
   echo -e "La base de datos db1 existe.\nSe creará otra base de datos para comparar archivos."
   ...
   # CREACION BASE DE DATOS 2
	...
	ls -liRu  --time-style=+%s $ruta | tr -s " " " " | tr -t " " ":" | cut -f1-7 -d: > datosacceso 2>> errores.log
	ls -liR   --time-style=+%s $ruta | tr -s " " " " | tr -t " " ":" | cut -f1,7 -d: > datosmodi 2>> errores.log
	ls -liRc  --time-style=+%s $ruta | tr -s " " " " | tr -t " " ":" | cut -f1,7 -d: > datosinodo 2>> errores.log
	paste -d : datosacceso datosmodi datosinodo > datos 2>> errores.log
	cut datos -f1-7,9,11 -d: | sed -e 's/rwx/7/g' -e 's/rw-/6/g' -e 's/r-x/5/g' -e 's/r--/4/g' -e  's/-wx/3/g' -e 's/-w-/2/g' | sed -e 's/--x/1/g' -e 's/---/0/g' -e '/^\./d' -e '/^:/d' -e '/^total/d' -e '/\./d' -e '/\//d'| sort -k1g -t":" > db2 2>> errores.log
	...
	diff temporaldb1 temporaldb2 >> bitacora.log 2>> errores.log
...
# DE NO EXISTIR LA BASE DE DATOS ENTONCES EMPIEZAN LAS CONSIDERACIONES PARA CREARLA
	...
	touch errores.log
	touch configuracion.cfg 2>> errores.log                      # SE CREA EL ARCHIVO DE CONFIGURACIONES
	...
	case $opcion in
		2)
			echo -e "#SISTEMA DE ARCHIVOS\nSISARCH=/home" >> configuracion.cfg 2>> errores.log
			elegir_banderas=1
		;;
		1) 
			echo -e "Especifique con la ruta absoluta del directorio el cual desea revisar.\nPor ejemplo: /home/usuario/\n"
			read ruta
			if [ -d $ruta ]; then
				...
				echo "\nEl directorio no existe. Vuelva a ejecutar el programa con una ruta válida\n"; exit 0;   # SI NO EXISTE ESE DIRECTORIO SE CIERRA EL PROGRAMA
			fi
		;;
		*) 
	...
	case $opcion in
		...
		;;
		1) 
			echo -e "\n Indique la ruta donde se generará el archivo bitacora.log, el cual contendra los datos de los archivos modificados.\nPor ejemplo: /home/usuario/\n"
			read camino
			if [ -d $camino ]; then
				echo -e "\nEl directorio existe. Sobre $ruta se hará la revisión del sistema de archivos."
	...
	cut datos -f1-7,9,11 -d: | sed -e 's/rwx/7/g' -e 's/rw-/6/g' -e 's/r-x/5/g' -e 's/r--/4/g' -e  's/-wx/3/g' -e 's/-w-/2/g' | sed -e 's/--x/1/g' -e 's/---/0/g' -e '/^\./d' -e '/^:/d' -e '/^total/d' -e '/\./d' -e '/\//d'| sort -k1g -t":" > db1 2>> errores.log
	...
	echo -e "\nLa base de datos ha sido creada satisfactoriamente. Se llama db1. Se encuentra en el mismo directorio del que ha ejecutado este programa.\n\nPara que se añadan datos a la bitácora vuelva a ejecutar el programa y consulte el archivo bitacora.log.\n\n\n\n"
	exit 0
